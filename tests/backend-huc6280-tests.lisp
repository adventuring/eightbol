;; tests/backend-huc6280-tests.lisp - HuC6280 backend statement tests
;;
;; The HuC6280 (Hudson Soft) is used in PC Engine / TurboGrafx-16.
;; Based on 65C02 with additional instructions (TAM, TMA, block transfers).
;; EIGHTBOL HuC6280 backend delegates to the 6502 family emission code.
;; Tests verify delegation and HuC6280-specific header generation.

(in-package :eightbol/test)

(fiveam:def-suite :backend-huc6280
  :description "HuC6280 backend statement emission")
(in-suite :backend-huc6280)

(defun huc6280-asm (stmt &key (class-id "Character") (slots nil) (consts nil) (pic nil) (types nil))
  "Compile STMT inside a minimal method for HuC6280, return assembly string."
  (let ((*standard-output* (make-broadcast-stream)))
    (compile-method-ast-with-tables
     `(:method :method-id "M" :statements (,stmt))
     class-id :huc6280
     :slot-table (or slots (make-hash-table :test 'equalp))
     :const-table (or consts (make-hash-table :test 'equalp))
     :pic-width-table (or pic (make-hash-table :test 'equalp))
     :type-table (or types (make-hash-table :test 'equalp)))))

(defun make-hash (&rest pairs)
  (let ((h (make-hash-table :test 'equalp)))
    (loop for (key value) on pairs by #'cddr
          do (setf (gethash key h) value))
    h))

;;;
;;;; Header and structure
;;;

(test huc6280/header-contains-cpu-label
  "HuC6280 output header contains 'generated by EIGHTBOL for huc6280'."
  (let ((asm (huc6280-asm '(:goback))))
    (is (search "generated by EIGHTBOL for huc6280" asm))))

(test huc6280/method-emits-block-structure
  "Method body opens with .block and ends with .bend."
  (let ((asm (huc6280-asm '(:goback))))
    (is (search ".block" asm))
    (is (search ".bend" asm))))

(test huc6280/method-emits-dispatch-label
  "Method dispatch label is MethodClassName."
  (let ((asm (huc6280-asm '(:goback) :class-id "Actor")))
    (is (search "MethodActorM" asm))))

;;;
;;;; Termination statements
;;;

(test huc6280/goback
  "GOBACK emits rts."
  (let ((asm (huc6280-asm '(:goback))))
    (is (search "rts" asm))))

(test huc6280/exit-method
  "EXIT METHOD emits rts."
  (let ((asm (huc6280-asm '(:exit-method))))
    (is (search "rts" asm))))

(test huc6280/stop-run
  "STOP RUN emits rts."
  (let ((asm (huc6280-asm '(:stop-run))))
    (is (search "rts" asm))))

;;;
;;;; MOVE statement
;;;

(test huc6280/move-literal-to-var
  "MOVE literal TO var emits lda # immediate and sta."
  (let ((asm (huc6280-asm '(:move :from 42 :to "X"))))
    (is (search "lda #42" asm))
    (is (search "sta" asm))))

(test huc6280/move-slot-of-self
  "MOVE slot OF Self loads via Self-relative address."
  (let ((slots (make-hash "HP" "Character")))
    (let ((asm (huc6280-asm '(:move :from (:of "HP" :self) :to "Dest") :slots slots)))
      (is (search "ldy #CharacterHP" asm))
      (is (search "lda (Self), y" asm)))))

(test huc6280/move-named-constant
  "MOVE named constant uses symbolic immediate."
  (let ((consts (make-hash "song--heal--id" 42)))
    (let ((asm (huc6280-asm '(:move :from "Song--Heal--ID" :to "X") :consts consts)))
      (is (search "lda # Song_Heal_ID" asm)))))

;;;
;;;; ADD / SUBTRACT
;;;

(test huc6280/add-literal-to-var
  "ADD literal TO var emits clc and adc."
  (let ((pic (make-hash "A" 1 "B" 1)))
    (let ((asm (huc6280-asm '(:add :from "A" :to "B") :pic pic)))
      (is (search "clc" asm))
      (is (search "adc" asm)))))

(test huc6280/add-16-bit
  "ADD of 2-byte values emits multibyte add sequence."
  (let ((pic (make-hash "A" 2 "B" 2)))
    (let ((asm (huc6280-asm '(:add :from "A" :to "B") :pic pic)))
      (is (search "adc" asm))
      (is (search "sta" asm)))))

(test huc6280/subtract
  "SUBTRACT emits sec and sbc."
  (let ((pic (make-hash "A" 1 "B" 1)))
    (let ((asm (huc6280-asm '(:subtract :from "A" :from-target "B") :pic pic)))
      (is (search "sec" asm))
      (is (search "sbc" asm)))))

(test huc6280/subtract-16-bit
  "SUBTRACT of 2-byte values emits multibyte subtract sequence."
  (let ((pic (make-hash "A" 2 "B" 2)))
    (let ((asm (huc6280-asm '(:subtract :from "A" :from-target "B") :pic pic)))
      (is (search "sbc" asm))
      (is (search "sta" asm)))))

;;;
;;;; COMPUTE
;;;

(test huc6280/compute-literal
  "COMPUTE target = expr emits load and store."
  (let ((pic (make-hash "X" 1)))
    (let ((asm (huc6280-asm '(:compute :target "X" :expression 42) :pic pic)))
      (is (search "lda #42" asm))
      (is (search "sta" asm)))))

;;;
;;;; SET
;;;

(test huc6280/set-literal
  "SET target TO value emits lda and sta."
  (let ((asm (huc6280-asm '(:set :target "X" :value 99))))
    (is (search "lda #99" asm))
    (is (search "sta" asm))))

;;;
;;;; IF / conditionals
;;;

(test huc6280/if-equal
  "IF a = b emits compare and branch."
  (let ((pic (make-hash "A" 1 "B" 1)))
    (let ((asm (huc6280-asm '(:if :condition (= "A" "B") :then ((:move :from 0 :to "X"))
                            :class-id "T") :pic pic)))
      (is (search "cmp" asm)))))

(test huc6280/if-greater
  "IF a > b emits compare and branch."
  (let ((pic (make-hash "A" 1 "B" 1)))
    (let ((asm (huc6280-asm '(:if :condition (:greater "A" "B") :then ((:move :from 0 :to "X"))
                            :class-id "T") :pic pic)))
      (is (search "cmp" asm)))))

(test huc6280/if-less
  "IF a < b emits compare and branch."
  (let ((pic (make-hash "A" 1 "B" 1)))
    (let ((asm (huc6280-asm '(:if :condition (:less "A" "B") :then ((:move :from 0 :to "X"))
                            :class-id "T") :pic pic)))
      (is (search "cmp" asm)))))

(test huc6280/if-else
  "IF/ELSE emits branch around else block."
  (let ((pic (make-hash "A" 1 "B" 1)))
    (let ((asm (huc6280-asm '(:if :condition (= "A" "B")
                                    :then ((:move :from 1 :to "X"))
                                    :else ((:move :from 2 :to "Y")))
                             :pic pic :class-id "T")))
      (is (search "bne" asm)))))

;;;
;;;; INVOKE
;;;

(test huc6280/invoke-self
  "INVOKE Self \"Method\" emits .CallMethod."
  (let ((asm (huc6280-asm '(:invoke :object "Self" :method "Connect")
                          :class-id "Character")))
    (is (search ".CallMethod" asm))
    (is (search "InvokeCharacterConnect" asm))))

(test huc6280/invoke-super
  "INVOKE SUPER emits .CallMethod to parent class."
  (let ((eightbol::*parent-classes* (make-hash "Character" "Actor"))
        (eightbol::*method-id* "Think")
        (eightbol::*class-id* "Character"))
    (let ((asm (huc6280-asm '(:invoke-super))))
      (is (search ".CallMethod" asm)))))

;;;
;;;; CALL
;;;

(test huc6280/call
  "CALL target emits jsr."
  (let ((asm (huc6280-asm '(:call :target "MoveDecalY"))))
    (is (search "jsr" asm))))

(test huc6280/call-acc
  "CALL...USING loads argument then jsr."
  (let ((asm (huc6280-asm '(:call-acc :target "Foo" :using 7))))
    (is (search "lda #7" asm))
    (is (search "jsr" asm))))

;;;
;;;; PERFORM
;;;

(test huc6280/perform-simple
  "PERFORM procedure emits jsr."
  (let ((asm (huc6280-asm '(:perform :procedure "Foo"))))
    (is (search "jsr Foo" asm))))

(test huc6280/perform-times
  "PERFORM TIMES n emits loop with decrement and branch."
  (let ((asm (huc6280-asm '(:perform :procedure "Foo" :times 3))))
    (is (search "jsr Foo" asm))))

(test huc6280/perform-until
  "PERFORM UNTIL condition emits condition-checked loop."
  (let ((asm (huc6280-asm '(:perform :procedure "Foo"
                                    :until (:greater "X" 0))
                          :class-id "T")))
    (is (search "jsr" asm))))

(test huc6280/perform-with-body-until
  "PERFORM UNTIL with inline :body emits condition-checked loop."
  (let ((asm (huc6280-asm '(:perform :until (= "Done" 0) :body ((:move :from 1 :to "X"))))))
    (is (search "lda Done" asm))
    (is (search "bne" asm))))

(test huc6280/perform-with-body-varying
  "PERFORM VARYING emits counter init, varying store, and increment."
  (let ((pic (make-hash "X" 1)))
    (let ((asm (huc6280-asm '(:perform :varying "X" :from 0 :by 1 :until (= "Done" 0)
                                    :body ((:move :from 1 :to "X")))
                         :pic pic)))
      (is (search "lda" asm)))))

;;;
;;;; EVALUATE
;;;

(test huc6280/evaluate-when
  "EVALUATE WHEN comparison emits compare and branch."
  (let ((pic (make-hash "X" 1)))
    (let ((asm (huc6280-asm '(:evaluate :subject "X"
                                      :when-clauses ((:when "A" ((:move :from 1 :to "X")))))
                             :pic pic :class-id "T")))
      (is (search "cmp" asm)))))

;;;
;;;; INSPECT
;;;

(test huc6280/inspect-tallying
  "INSPECT TALLYING emits counting loop."
  (let ((pic (make-hash "Buf" 64 "Cnt" 1)))
    (let ((asm (huc6280-asm '(:inspect :target "Buf" :tallying "Cnt") :pic pic)))
      (is (search "lda" asm)))))

(test huc6280/inspect-replacing
  "INSPECT REPLACING emits fill loop."
  (let ((pic (make-hash "Buf" 64)))
    (let ((asm (huc6280-asm '(:inspect :target "Buf" :by 42) :pic pic)))
      (is (search "lda" asm)))))

;;;
;;;; GOTO / PARAGRAPH
;;;

(test huc6280/goto
  "GO TO target emits bra to label (HUC6280 has BRA)."
  (let ((asm (huc6280-asm '(:goto :target "Exit"))))
    (is (search "bra" asm))))

(test huc6280/paragraph
  "Paragraph emits label."
  (let ((asm (huc6280-asm '(:paragraph "MyPara"))))
    (is (search "MyPara:" asm))))

;;;
;;;; LOG FAULT / DEBUG BREAK
;;;

(test huc6280/log-fault
  "LOG FAULT emits comment."
  (let ((asm (huc6280-asm '(:log-fault :code 1234))))
    (is (search "LOG FAULT" asm))))

(test huc6280/debug-break
  "DEBUG BREAK emits comment."
  (let ((asm (huc6280-asm '(:debug-break :code 42))))
    (is (search "DEBUG BREAK" asm))))

;;;
;;;; STRING BLT
;;;

(test huc6280/string-blt
  "STRING dest FROM src emits block copy loop."
  (let ((slots (make-hash "Src" "Character" "Dst" "Character")))
    (let ((asm (huc6280-asm '(:string-blt :source "Src" :dest "Dst") :slots slots)))
      (is (search "ldy #$00" asm))
      (is (search "lda" asm))
      (is (search "sta" asm)))))

;;;
;;;; UNSUPPORTED
;;;

(test huc6280/divide-signals-error
  "DIVIDE on HuC6280 signals backend-error."
  (signals eightbol::backend-error
    (huc6280-asm '(:divide :from "A" :into "B"))))

(test huc6280/multiply-signals-error
  "MULTIPLY on HuC6280 signals backend-error."
  (signals eightbol::backend-error
    (huc6280-asm '(:multiply :by "A" :on "B"))))
