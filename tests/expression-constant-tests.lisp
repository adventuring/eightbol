...
(test expr-const/subtract-from-constant
  "Expression (:- :subtrahend 1 :from \"MapWidth\") should not be constant (MapWidth is variable)."
  (let ((expr '(:- :subtrahend 1 :from "MapWidth")))
...
(test expr-const/subtract-constant-from-constant
  "Expression (:- :subtrahend 1 :from 20) should be constant."
  (let ((expr '(:- :subtrahend 1 :from 20)))
...
(test expr-const/nested-arithmetic
  ...
  (let ((expr '(:- :subtrahend 1
                   :from (:× :by 2
                           :multiplier (:+ :from 2 :to 1)))))
    (is (eightbol::expression-constant-p expr))
...
(test expr-const/complex-boat-expression
  ...
  (let* ((width-of-self '(:of "Width" :self))
         ...
         (inner-expr '(:- :subtrahend 1
                          :from (:× :by 2
                                  :multiplier (:+ :from (:+ :from (:∧ width-of-self 3) :to 1) :to 0))))
         ;; Width-Mask BIT-AND inner-expr
         (bit-and-expr `(:∧ ,width-mask ,inner-expr))
         ;; Width-Mask BIT-XOR bit-and-expr
         (bit-xor-expr `(:⊻ ,width-mask ,bit-and-expr))
         ;; 5 SHIFT-LEFT 5
         (shift-expr '(:ash 5 5))
         ;; Final: bit-xor-expr BIT-OR shift-expr
         (full-expr `(:∨ ,bit-xor-expr ,shift-expr)))
