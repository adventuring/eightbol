...
Used when after PIC/PICTURE: treat as picture-sequence instead of expanding as subscript."
  (and (stringp token-str)
       ...
       (let ((c0 (char token-str 0)))
         (or (digit-char-p c0)
             (find c0 ".XxAaBbVvPpSsZzNn") ; common PIC type chars
             ))
        ...
        (dolist (token (or (when (and (every #'constituent-char-p token)
                           ...
                           (list (append (cond
                                           ...
                                           ((member (string token) (mapcar #'string (token-list))
                                                    :test #'string-equal)
                                            (let ((*package* (find-package :eightbol)))
                                              (list (intern (string-upcase token)) token)))
