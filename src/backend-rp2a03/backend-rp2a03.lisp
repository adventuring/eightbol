  ...
  (let* ((class-id (getf (rest ast) :class-id))
    ...
    (multiple-value-bind (slot-table type-table const-table service-bank-table usage-table sign-table
      ...
      (let ((*output-stream* output-stream)
            ...
            (*z80-continue-label* nil)
            (*6502-continue-label* nil)
        ...
        (dolist (method (ensure-list methods))
          (when (and (listp method) (eq (first method) :method))
            (emit-one-6502-family-method output-stream class-id method :rp2a03
                                         (lambda (m cid cpu)
                                           (declare (ignore cpu))
                                           (compile-rp2a03-method m cid)))))))))
...
Loads value into accumulator and calls print handler."
  (let ((value (getf ast-node-data :value)))
    ...
    (format *output-stream* "~%~10Tlda ~a" (emit-6502-value value))
    ;; Call print output routine
    (format *output-stream* "~%~10Tjsr DoPrint~%")))

(def-rp2a03-statement :input
...
Reads input and stores into target identifier."
  (let ((target (getf ast-node-data :target)))
    ...
    (format *output-stream* "~%~10Tsta ~a~%" (emit-6502-value target))))

(def-rp2a03-statement :service-bank
  (declare (ignore ast-node-data))
