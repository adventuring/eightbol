...
Fountain uses: :scene, :dialogue, :action, :transition, :character-entry, :set, :if, :print, :input
...
SCRIPT-NAME is used to generate the Forth word name (defaults to 'Script' if nil)."
   ...
   (cond
     ...
     ;; :dialogue node - canonical :dialogue
     ...
     ;; :set node - already canonical
     ((and (listp fountain-ast) (eq (first fountain-ast) :set))
      fountain-ast)
    
     ;; :if node - already canonical
     ((and (listp fountain-ast) (eq (first fountain-ast) :if))
