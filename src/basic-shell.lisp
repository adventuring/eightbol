            ...
            (let ((cmd (basic-shell-command-keyword line)))
              (cond
                ...
                ((string-equal cmd "HELP")
                 (format *query-io* "~&Commands: NEW [\"name\"], OLD [\"name\"], SAVE [\"name\"], LIST [\"name\"], RENUM [start[,inc]], RUN, BYE, SCRATCH, CATALOG, RENAME [\"name\"], UNSAVE [\"name\"], HELP.~%"))
                ...
                ((string-equal cmd "NEW")
                 ...
                 (let ((rest (basic-shell-command-rest line)))
                   (cond
                     ((plusp (length rest))
                      (setf (getf workspace :class-id)
                            (header-case (basic-parse-quoted-name rest)))
                      (write-basic-file-text (basic-path-for-class (getf workspace :class-id) root) "")
                      (format *query-io* "~&New program ~A.bas~%" (getf workspace :class-id)))
                     (t
                      (setf (getf workspace :class-id) nil)
