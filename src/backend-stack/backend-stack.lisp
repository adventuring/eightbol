...
(def-stack-statement :move (ast-node-data)
  (let ((from (getf ast-node-data :from))
    ...
    (cond
      ((and (listp from) (eq (first from) :on))
       (let ((slot (second from)) (obj (third from)))
         (format *output-stream* "~10T; MOVE ON ~a to ~a~%" from to)
         (format *output-stream* "~10Tpush ~a~%" obj)
         (format *output-stream* "~10Tload-slot ~a~%" slot)))
      ((and (listp from) (eq (first from) :of))
       (let ((slot (second from)) (obj (third from)))
         (format *output-stream* "~10T; MOVE OF ~a to ~a~%" from to)
         (format *output-stream* "~10Tpush ~a~%" obj)
         (format *output-stream* "~10Tload-slot ~a~%" slot)))
      (t
