  ...
  (cond
    ...
    ((and (listp condition) (member (first condition) '(< less > greater) :test #'eq))
     ...
     (ecase (first condition)
       ((< less) (format *output-stream* "  if_icmpge ~a~%" false-label))
       ((> greater) (format *output-stream* "  if_icmple ~a~%" false-label))))
    ((and (listp condition) (member (first condition) '(:≠ :≤ :≥) :test #'eq))
     (compile-jvm-load (second condition))
     (compile-jvm-load (third condition))
     (ecase (first condition)
       ((:≠) (format *output-stream* "  if_icmpeq ~a~%" false-label))
       ((:≤) (format *output-stream* "  if_icmpgt ~a~%" false-label))
       ((:≥) (format *output-stream* "  if_icmplt ~a~%" false-label))))
    (t
