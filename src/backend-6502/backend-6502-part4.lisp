  ...
  (let* ((object (safe-getf (rest statement) :object))
    ...
    (cond
      ((string-equal method "Class-P")
       (when using
         (emit-6502-load-expression out using class-id))
...
(defun emit-6502-false-when-not-unsigned-greater-than-zero (out lhs class-id branch-label)
   "Emit code that jumps to BRANCH-LABEL when signed LHS is not greater than or equal to zero.
   
   For rhs zero, signed >= 0 is equivalent to “sign bit clear”.
   Uses lda highest byte + bmi branch if negative."
   (let ((w (max 1 (operand-width lhs))))
     (emit-6502-load-byte-n out lhs class-id (1- w) w)
     (format out "~%~10Tbmi ~a~%" branch-label)))
