...
(defun literal-minus-one-p (expression)
  "True when EXPRESSION is the integer -1, string \"-1\", or (:literal -1)."
  (or (eql expression -1)
      (equalp expression '(:literal -1))))
...
;; Optimise: ADD 1 TO variable (no GIVING) → inc variable (byte only)
      ...
      ;; Optimise: ADD -1 TO variable (no GIVING) → dec variable (byte only)
       ((and (literal-minus-one-p from) (not giving) (stringp to-op) (= (operand-width to-op) 1))
        (format out "~%~10Tdec ~a" (emit-6502-value to-op))
        (setf *6502-accumulator-expression* :trash/-1))
      ;; Optimise: ADD 2,4,8,16,32,64,128 TO variable (no GIVING) → asl/rol chain (byte only)
       ((and (power-of-two-p (expression-constant-value from)) (not giving) (stringp to-op) (= (operand-width to-op) 1))
