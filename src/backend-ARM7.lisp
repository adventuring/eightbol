;; src/backend-ARM7.lisp — ARM7TDMI code generation backend
;;
;; The ARM7TDMI supports ARM (32-bit) and Thumb (16-bit) instruction sets.
;; Registers: R0–R15 (R13=SP, R14=LR, R15=PC)
;; Instructions: ldr, str, mov, add, sub, cmp, b, bl, bx
;;
;; Output: ARM assembly (GAS syntax). Uses Thumb for smaller code.
;; Uses: R0=accumulator, R1=temp, R2=ptr
(in-package :eightbol)

(defun arm7-symbol (name)
  "Convert EIGHTBOL identifier to ARM assembly symbol (PascalCase)."
  ...
  (let* ((class-id (getf (rest ast) :class-id))
    ...
    (multiple-value-bind (slot-table type-table const-table service-bank-table usage-table sign-table pic-size-table pic-width-table)
      ...
      (format output-stream
              "@ ~a -- generated by EIGHTBOL for ARM7TDMI~%@ DO NOT EDIT -- regenerate from ~a.cob~2%"
              class-id class-id)
...
(defun compile-arm7-statement (out stmt class-id slot-table type-table const-table pic-size-table pic-width-table)
  ...
  (ecase (first stmt)
    ...
    (:goto
     (format out  "~&~8tb       ~a" (arm7-symbol (format nil "~a" (or (getf (rest stmt) :target) (first (getf (rest stmt) :targets)))))))
    (:paragraph
  ...
  (let ((op (arm7-load-op width)))
    (cond
      ...
      ((and (stringp expr) (constant-p expr const-table))
       (format out  "~&~8tmovs    r0, #~d" (constant-value expr const-table)))
      ((stringp expr)
      ...
      ((and (listp expr) (eq (first expr) :of))
       (let ((slot (second expr)) (obj (third expr)))
         (when (member obj '(:self "Self" self) :test #'equal)
           ...
           (format out  "~&~8t~a     r0, [r2, #~a]" op (slot-symbol slot slot-table class-id)))))
      ...
      ((and (listp expr) (eq (first expr) :bit-and))
       ...
       (let ((rhs (third expr)))
         (if (arm7-expr-is-constant-p rhs const-table)
             (format out  "~&~8tands    r0, r0, #~d" (if (numberp rhs) rhs (constant-value rhs const-table)))
      ...
      ((and (listp expr) (eq (first expr) :bit-or))
       ...
       (let ((rhs (third expr)))
         (if (arm7-expr-is-constant-p rhs const-table)
             (format out  "~&~8torrs    r0, r0, #~d" (if (numberp rhs) rhs (constant-value rhs const-table)))
             (progn (format out  "~&~8tmovs    r1, r0")
                    (compile-arm7-load out rhs class-id slot-table const-table pic-width-table)
      ...
      ((and (listp expr) (eq (first expr) :bit-xor))
       ...
       (let ((rhs (third expr)))
         (if (arm7-expr-is-constant-p rhs const-table)
             (format out  "~&~8teors    r0, r0, #~d" (if (numberp rhs) rhs (constant-value rhs const-table)))
             (progn (format out  "~&~8tmovs    r1, r0")
                    (compile-arm7-load out rhs class-id slot-table const-table pic-width-table)
       ...
       (let ((slot (second to-dest)) (obj (third to-dest)))
         (when (member obj '(:self "Self" self) :test #'equal)
           ...
           (format out  "~&~8t~a     r0, [r2, #~a]"
                   op (slot-symbol slot slot-table class-id)))))
