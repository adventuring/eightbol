...
(defun compile-m6800-condition (condition false-label)
  (cond
    ((and (listp condition) (member (first condition) '(= equal < less > greater :neq :le :ge :≠ :≤ :≥) :test #'eq))
     (let ((op (first condition))
           (lhs (second condition))
           (rhs (third condition)))
       (cond
         ((or (eq op '=) (eq op 'equal))
          (compile-m6800-load-a lhs)
          (format *output-stream*  "~&~8tTAB")
          (compile-m6800-load-a rhs)
          (format *output-stream*  "~&~8tCMA")
         ...
         ((or (eq op '<) (eq op 'less))
          ...
          (compile-m6800-load-a rhs)
          (format *output-stream*  "~&~8tCMA")
          (format *output-stream*  "~&~8tBCS     ~a" false-label))
         ((or (eq op '>) (eq op 'greater))
          (compile-m6800-load-a lhs)
          (format *output-stream*  "~&~8tTAB")
          (compile-m6800-load-a rhs)
          (format *output-stream*  "~&~8tCMA")
          (compile-m6800-load-a lhs)
          (format *output-stream*  "~&~8tCMA")
          (format *output-stream*  "~&~8tBC     ~a" false-label))
         ((or (eq op :≠) (eq op 'neq))
          (compile-m6800-load-a lhs)
          (format *output-stream*  "~&~8tTAB")
          (compile-m6800-load-a rhs)
         ...
         ((or (eq op :≤) (eq op 'le))
          ...
          (format *output-stream*  "~&~8tBEQ     ~a" false-label))
          ((or (eq op :≥) (eq op 'ge))
           (format *output-stream*  "~&~8t; :≥ needs subtraction comparison")))))
