  ...
  (let ((*6502-family-cpu* cpu)
    ...
    (let* ((*class-id* (safe-getf (rest ast) :class-id))
      ...
      (format output-stream
              ...
              *class-id* cpu-label (string-capitalize (param-case *class-id*)))
...
@code{#$nn}, matching @code{emit-6502-load-expr}."
  ...
  (let ((expr (%move-from-resolve-constant-of-self expr)))
    ...
    (cond
      ...
      ((and (slot-of-expr expr) (not (slot-of-self-p expr)))
       (let* ((sof (slot-of-expr expr))
              (off (slot-symbol (second sof) (third sof)))
              (ptr (6502-object-pointer-label (third sof) class-id)))
         (format out "~&~10Tldy # ~a~[~:;~:* + ~d~]" off n)
      ...
      ((stringp expr)
       (format out "~&~10Tlda ~a~[~:;~:* + ~d~]"
               (bare-data-assembly-symbol expr class-id) n))
      (t
       (format out "~&~10Tlda ~a~[~:;~:* + ~d~]"
  ...
  (cond
    ...
    ((stringp dest)
     (format out "~&~10Tsta ~a~[~:;~:* + ~d~]"
             (bare-data-assembly-symbol dest class-id) n))
    (t
     (format out "~&~10Tsta ~a~[~:;~:* + ~d~]"
    ...
    (let* ((condition (or (normalize-relation-condition condition) condition))
      ...
      (cond
        ...
        ((and (member op '(= equal) :test #'string=)
         ...
         (let ((ptr-expr (if (string-equal (princ-to-string (second condition)) :null)
                             (third condition)
                             (second condition))))
        ...
        ((and (string-equal op :not)
         ...
         (let* ((inner (second condition))
                (ptr-expr (if (string-equal (princ-to-string (second condition)) :null)
                              (third inner)
                              (second inner))))
           (emit-6502-load-hi-byte out ptr-expr class-id)
           (format out "~&~10Tbeq ~a" branch-label)))
