...
(defun compile-i286-method (out method class-id slot-table type-table const-table pic-size-table pic-width-table)
  (let ((*slot-table* slot-table)
    ...
    (let* ((method-id (getf (rest method) :method-id))
           ...
           (proc-label (if custom-entry
                           (i286-symbol custom-entry)
                           dispatch-label))
           (statements (nth-value 1 (split-method-leading-assembly-entry
                                      (getf (rest method) :statements)))))
      (format out "~&~a proc" proc-label)
      ...
      (format out "~&~8tret")
      (format out "~&~a endp" proc-label)
      (when custom-entry
        (format out "~&~a = ~a" dispatch-label proc-label)))))
...
(defun compile-i286-statement (out stmt class-id slot-table type-table const-table pic-size-table pic-width-table)
  ...
  (ecase (first stmt)
    ...
    (:+ (compile-i286-add out stmt class-id slot-table const-table pic-width-table))
    (:- (compile-i286-subtract out stmt class-id slot-table const-table pic-width-table))
    ...
    (:ash
     ...
     (:dialogue
      (let ((text (getf (rest stmt) :text))
            (speaker (or (getf (rest stmt) :speaker) "Narrator")))
        ;; Emit dialogue ID reference (deduplication handled upstream)
        (format out "~&~8t; Dialogue: ~a | ~a" text speaker)
        ;; Load dialogue ID into AL (ID generated by asset pipeline)
        (format out "~&~8tmov     al, DialogueID_~a"
                (normalize-identifier-to-pascal-case text))
        ;; Set speaker register (BL holds speaker ID)
        (format out "~&~8tmov     bl, ~a" (normalize-identifier-to-pascal-case speaker))
        ;; Call dialogue dispatcher
        (format out "~&~8tcall    DoDialogue~%")))
     (:print
      (let ((value (getf (rest stmt) :value)))
        (format out "~&~8t; Print: ~a" value)
        ;; Load value into AL
        (compile-i286-load out value class-id slot-table const-table pic-width-table)
        ;; Call print output routine
        (format out "~&~8tcall    DoPrint~%")))
     (:input
      (let ((target (getf (rest stmt) :target)))
        (format out "~&~8t; Input to ~a" target)
        ;; Call input routine (returns value in AL)
        (format out "~&~8tcall    DoInput")
        ;; Store result to target
        (when (stringp target)
          (format out "~&~8tmov     ~a, al~%" (i286-symbol target)))))
...
((and (listp expr) (eq (first expr) :of))
      ...
      ((and (listp expr) (eq (first expr) :ash))
       (let ((n (if (numberp (third expr)) (third expr) 1)))
         ...
         (if (minusp n)
             (dotimes (_ (abs n)) (format out "~&~8tshr     ~a, 1" reg))
  ...
  (cond
    ...
    ((and (listp condition) (member (first condition) '(:≠ :≤ :≥) :test #'eq))
     (let ((lhs (second condition)) (rhs (third condition)) (op (first condition))
       ...
       (ecase op
         ...
         ((:≤) (format out "~&~8tjg      ~a" branch-label))
         ((:≥) (format out "~&~8tjl      ~a" branch-label)))))
    ...
    ((and (listp condition) (eq (first condition) :or))
     (let ((lbl-cond2 (i286-label "or"))
       ...
       (compile-i286-condition out (second condition) class-id slot-table type-table const-table pic-width-table branch-label lbl-cond2)
...
(defun compile-i286-add (out stmt class-id slot-table const-table pic-width-table)
  (let* ((from (getf (rest stmt) :from))
         ...
         (acc (i286-reg w))
         (tmp (i286-temp-reg w)))
    (assert-pic-decimal-add-compiled :i286 stmt)
    (when (and bcd-p (> (or w 1) 2))
      (dotimes (i w)
        (format out "~&~8tmov     al, byte ptr ~a+~d" (i286-symbol from) i)
        (if (= i 0) (format out "~&~8tadd     al, byte ptr ~a+~d" (i286-symbol to) i)
            (format out "~&~8tadc     al, byte ptr ~a+~d" (i286-symbol to) i))
        (format out "~&~8tdaa")
        (format out "~&~8tmov     byte ptr ~a+~d, al" (i286-symbol result) i))
      (return-from compile-i286-add))
    (backend-unsupported-operand-width :i286 :+ w 2)
...
(defun compile-i286-subtract (out stmt class-id slot-table const-table pic-width-table)
  (multiple-value-bind (minuend subtrahend)
    ...
    (let* ((giving (getf (rest stmt) :giving))
           ...
           (bcd-p (when dest (operand-bcd-p dest)))
           (acc (i286-reg w))
           (tmp (i286-temp-reg w)))
      (assert-pic-decimal-subtract-compiled :i286 stmt)
      (when (and bcd-p (> (or w 1) 2))
        (dotimes (i w)
          (format out "~&~8tmov     al, byte ptr ~a+~d" (i286-symbol minuend) i)
          (if (= i 0) (format out "~&~8tsub     al, byte ptr ~a+~d" (i286-symbol subtrahend) i)
              (format out "~&~8tsbb     al, byte ptr ~a+~d" (i286-symbol subtrahend) i))
          (format out "~&~8tdas")
          (format out "~&~8tmov     byte ptr ~a+~d, al" (i286-symbol dest) i))
        (return-from compile-i286-subtract))
      (backend-unsupported-operand-width :i286 :- w 2)
      (compile-i286-load out minuend class-id slot-table const-table pic-width-table)
      (format out "~&~8tmov     ~a, ~a" tmp acc)
      (compile-i286-load out subtrahend class-id slot-table const-table pic-width-table)
