;; src/backend-m6800.lisp — Motorola 6800 code generatio      (when (and bcd-p (> (or w 1) 2))
        (dotimes (i w)
          (format out "~&~8tLDAA    ~a+~d" (m6800-symbol minuend) i)
          (format out "~&~8tTAB")
          (format out "~&~8tLDAA    ~a+~d" (m6800-symbol subtrahend) i)
          (if (= i 0) (format out "~&~8tSBA") (format out "~&~8tSBCA    #0"))
          (let ((ok1 (m6800-label "ms1")) (ok2 (m6800-label "ms2")))
            (format out "~&~8tTAB")
            ...
            (format out "~&~a:" ok2)
            (format out "~&~8tLDAA    B"))
          (format out "~&~8tSTAA    ~a+~d" (m6800-symbol result) i))
    ...
    (:add
     (let* ((to (getf (rest stmt) :to))
       ...
       (when (and bcd-p (> (or w 1) 2))
         (dotimes (i w)
           (format out "~&~8tLDAA    ~a+~d" (m6800-symbol from) i)
           (format out "~&~8tTAB")
           (format out "~&~8tLDAA    ~a+~d" (m6800-symbol to) i)
           (if (= i 0) (format out "~&~8tABA") (format out "~&~8tADCA   #0"))
           (let ((ok1 (m6800-label "ma1")) (ok2 (m6800-label "ma2")))
             (format out "~&~8tTAB")
             (format out "~&~8tANDA    #$0F")
             (format out "~&~8tCMPA    #10")
             (format out "~&~8tBLO     ~a" ok1)
     ...
     (assert-pic-decimal-subtract-compiled :m6800 stmt)
      (when (and bcd-p (> (or w 1) 2))
        (dotimes (i w)
          ...
          (format out "~&~8tANDA    #$0F")
          (format out "~&~8tCMPA    #10")
          (let ((sk (m6800-label "ms")))
            (format out "~&~8tBLO     ~a" sk)
            (format out "~&~8tLDAA    B")
            (format out "~&~8tSUBA    #6")
            (format out "~&~8tTAB")
            (format out "~&~a:" sk))
