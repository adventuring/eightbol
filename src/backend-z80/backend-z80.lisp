...
(defun compile-z80-statement (out statement class-id slot-table type-table const-table pic-size-table pic-width-table)
  ...
  (ecase (first statement)
    ...
    (:perform
     (compile-z80-perform out statement class-id slot-table type-table const-table pic-size-table pic-width-table))
    (:break
     (if *z80-break-label*
    ...
    (:dialogue
      (let ((text (getf (rest statement) :text))
            (speaker (or (getf (rest statement) :speaker) "Narrator")))
        ;; Emit dialogue ID reference (deduplication handled upstream)
        (format out "~&~10t;; Dialogue: ~a | ~a" text speaker)
        ;; Load dialogue ID into A (ID generated by asset pipeline)
        (format out "~&~10tld a, DialogueID_~a"
                (normalize-identifier-to-pascal-case text))
        ;; Set speaker register (B holds speaker ID)
        (format out "~&~10tld b, ~a" (normalize-identifier-to-pascal-case speaker))
        ;; Call dialogue dispatcher
        (format out "~&~10tcall DoDialogue~%")))
     (:print
      (let ((value (getf (rest statement) :value)))
        (format out "~&~10t;; Print: ~a" value)
        ;; Load value into A
        (compile-z80-load out value class-id slot-table const-table pic-width-table)
        ;; Call print output routine
        (format out "~&~10tcall DoPrint~%")))
     (:input
      (let ((target (getf (rest statement) :target)))
        (format out "~&~10t;; Input to ~a" target)
        ;; Call input routine (returns value in A)
        (format out "~&~10tcall DoInput")
...
((and (listp expr) (eq (first expr) :of))
      (let ((obj (third expr)))
        (when (member obj '(:self "Self" self) :test #'equal)
          ...
          (format out "~&~10tld de, #~a" (apply #'slot-symbol (rest expr)))
     ...
     ((and (listp expr) (eq (first expr) :on))
      (let ((obj (third expr)))
        (when (member obj '(:self "Self" self) :test #'equal)
          (format out "~&~10tld hl, ~a + ~a" (bare-data-assembly-symbol obj class-id) (apply #'slot-symbol (rest expr)))
          (if (= (or width 1) 2)
              (progn
                (format out "~&~10tld a, (hl)")
                (format out "~&~10tinc hl")
         ...
         (when (member obj '(:self "Self" self) :test #'equal)
           (if (= (or to-w 1) 2)
               (progn
                 ...
                 (format out "~&~10tld hl, (Self)")
                 (format out "~&~10tld de, #~a" (apply #'slot-symbol (rest to)))
                 (format out "~&~10tadd hl, de")
...
(defun compile-z80-if (out statement class-id slot-table type-table const-table pic-size-table pic-width-table)
  (let ((condition (getf (rest statement) :condition))
        ...
        (label-else (z80-label "else"))
        (label-end (z80-label "end")))
    (compile-z80-condition out condition class-id slot-table type-table const-table pic-width-table label-else)
...
(defun compile-z80-condition (out condition class-id slot-table type-table const-table pic-width-table branch-label)
  (cond
    ...
    ((and (listp condition) (eq (first condition) :or))
     (let ((label-cond2 (z80-label "or"))
           (label-skip (z80-label "skip")))
       (compile-z80-condition out (second condition) class-id slot-table type-table const-table pic-width-table label-cond2)
       (format out "~&~10tjp ~a" label-skip)
             ...
             ((or (eq phrases 'eightbol::true) (equal phrases "TRUE"))
              ...
              (if (= (or (operand-width subject pic-width-table) 1) 2)
                  (progn (format out "~&~10tld a, h")
                         (format out "~&~10tor l"))
                  (format out "~&~10tor a"))
              (format out "~&~10tjp z, ~a" label-next)
              (dolist (s (ensure-list statements))
                (compile-z80-statement out s class-id slot-table type-table const-table pic-size-table pic-width-table))
              (format out "~&~10tjp ~a" label-end)
             ...
             ((or (and (symbolp phrases) (string= (symbol-name phrases) "FALSE"))
              ...
              (if (= (or (operand-width subject pic-width-table) 1) 2)
                  (progn (format out "~&~10tld a, h")
                         (format out "~&~10tor l"))
                  (format out "~&~10tor a"))
              (format out "~&~10tjp nz, ~a" label-next)
              (dolist (s (ensure-list statements))
                (compile-z80-statement out s class-id slot-table type-table const-table pic-size-table pic-width-table))
              (format out "~&~10tjp ~a" label-end)
              (format out "~&~a:" label-next))
      ...
      (cond
        (tally
         (let ((tally-sym (z80-symbol (format nil "~a" tally))))
           ...
           (let ((label (z80-label "tally"))
                 (label-done (z80-label "tallyd"))
                 (label-skip (z80-label "tallys")))
             (format out "~&~a:" label)
             (format out "~&~10tld a, (hl)")
             (format out "~&~10tor a")
             ...
             (format out "~&~a:" label-skip)
             (format out "~&~10tinc hl")
             (format out "~&~10tjp ~a" label)
             (format out "~&~a:" label-done))))
        (conv-from
         (format out "~&~10t;; INSPECT ~a CONVERTING" target)
         (format out "~&~10tld hl, ~a" tgt-sym)
         (let ((label (z80-label "insp"))
               (label-next (z80-label "inspn"))
               (label-done (z80-label "inspd")))
           (format out "~&~a:" label)
           (format out "~&~10tld a, (hl)")
           (format out "~&~10tor a")
           (format out "~&~10tjp z, ~a" label-done)
  ...
  (let* ((from (getf (rest statement) :from))
    ...
    (if (and bcd-p (> (or w 1) 2))
        (progn
          (format out "~&~10tand a")
          (dotimes (i w)
            (format out "~&~10tld a, (~a+~d)" (bare-data-assembly-symbol from class-id) i)
            (format out "~&~10tld b, a")
            (format out "~&~10tld a, (~a+~d)" (bare-data-assembly-symbol to class-id) i)
            (format out "~&~10tadc a, b")
            (format out "~&~10tdaa")
    ...
    (let* ((giving (getf (rest statement) :giving))
      ...
      (if (and bcd-p (> (or w 1) 2))
          (progn
            (format out "~&~10tscf")
            (dotimes (i w)
              (format out "~&~10tld a, (~a+~d)" (bare-data-assembly-symbol minuend class-id) i)
              (format out "~&~10tld b, a")
              (format out "~&~10tld a, (~a+~d)" (bare-data-assembly-symbol subtrahend class-id) i)
              (format out "~&~10tsbc a, b")
              (let ((sk1 (z80-label "bcs")) (sk2 (z80-label "bcs2")))
                (format out "~&~10tjr nc, ~a" sk1)
                (format out "~&~10tsub $60")
                (format out "~&~a:" sk1)
                (format out "~&~10tld b, a")
                (format out "~&~10tand $0F")
                (format out "~&~10tcp 10")
                (format out "~&~10tjr c, ~a" sk2)
                (format out "~&~10tld a, b")
                (format out "~&~10tsub $06")
                (format out "~&~10tld b, a")
                (format out "~&~a:" sk2)
                (format out "~&~10tld a, b"))
          ...
          (progn
            ...
            (if (= (or w 1) 2)
                ...
                (progn
                  ...
                  (format out "~&~10tsub c")
                  (when bcd-p
                    (let ((sk1 (z80-label "bss1")) (sk2 (z80-label "bss2")))
                      (format out "~&~10tjr nc, ~a" sk1)
                      (format out "~&~10tsub $60")
                      (format out "~&~a:" sk1)
                      (format out "~&~10tld b, a")
            ...
            (when (member obj '(:self "Self" self) :test #'equal)
              (if (= (or w 1) 2)
                  (progn
                    (format out "~&~10tex de, hl")
                    (format out "~&~10tld hl, (Self)")
                    (format out "~&~10tld bc, #~a" (apply #'slot-symbol (rest target)))
                    (format out "~&~10tadd hl, bc")
                    (format out "~&~10tld (hl), e")
                    (format out "~&~10tinc hl")
                    (format out "~&~10tld (hl), d"))
                  (progn
                    (format out "~&~10tld hl, (Self)")
                    (format out "~&~10tld de, #~a" (apply #'slot-symbol (rest target)))
                    (format out "~&~10tadd hl, de")
                    (format out "~&~10tld (hl), a"))))))
